...
<div class="container-fluid px-4">
    <h1 class="mt-4">Modifier une Salle</h1>
            ...
            <form action="{{ route('admin.salles.update', $salle) }}" method="POST">
                ...
                <div class="mb-3">
                    ...
                    <select class="form-select @error('type') is-invalid @enderror" 
                        ...
                        <option value="Salle de cours" {{ old('type', $salle->type) == 'Salle de cours' ? 'selected' : '' }}>Salle de cours</option>
                        <option value="Laboratoire" {{ old('type', $salle->type) == 'Laboratoire' ? 'selected' : '' }}>Laboratoire</option>
                        <option value="Amphithéâtre" {{ old('type', $salle->type) == 'Amphithéâtre' ? 'selected' : '' }}>Amphithéâtre</option>
                        <option value="Salle informatique" {{ old('type', $salle->type) == 'Salle informatique' ? 'selected' : '' }}>Salle informatique</option>
                        <option value="Autre" {{ old('type', $salle->type) == 'Autre' ? 'selected' : '' }}>Autre</option>
                    </select>
                    @error('type')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                ...
                <div class="mb-3">
                    ...
                    <input type="number" class="form-control @error('capacite') is-invalid @enderror" 
                           id="capacite" name="capacite" value="{{ old('capacite', $salle->capacite) }}" min="1"
                ...
                <div class="mb-3">
                    ...
                    <textarea class="form-control @error('description') is-invalid @enderror" 
                              ...
                              placeholder="Description optionnelle de la salle">{{ old('description', $salle->description) }}</textarea>
                    @error('description')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
