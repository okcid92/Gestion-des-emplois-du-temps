...
return new class extends Migration
{
    public function up()
    {
        ...
        // Convertir les dates existantes en jours de la semaine en anglais
        DB::statement("
            UPDATE schedules 
            SET new_day = CASE 
                WHEN DAYNAME(day) = 'Monday' THEN 'Monday'
                WHEN DAYNAME(day) = 'Tuesday' THEN 'Tuesday'
                WHEN DAYNAME(day) = 'Wednesday' THEN 'Wednesday'
                WHEN DAYNAME(day) = 'Thursday' THEN 'Thursday'
                WHEN DAYNAME(day) = 'Friday' THEN 'Friday'
                WHEN DAYNAME(day) = 'Saturday' THEN 'Saturday'
                WHEN DAYNAME(day) = 'Sunday' THEN 'Sunday'
            END
        ");
    ...
    {
        Schema::table('schedules', function (Blueprint $table) {
            // Créer une colonne temporaire de type date
            $table->date('new_day')->nullable();
        });

        // Convertir les jours de la semaine en dates
        DB::statement("
            UPDATE schedules 
